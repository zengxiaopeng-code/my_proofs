...
\begin{itemize}
  ...
  \par\smallskip\emph{Why it is not discharged.} \textbf{Now sharpened.} The equation has been split: \emph{availability} of stopping and of deferring already gives $\max\{V_t,\text{defer}\}\le U_t$ as a \textbf{theorem} (\texttt{DMC.Paper.le\_stopping\_max}). What is still assumed is only the converse, i.e.\ \emph{exhaustiveness}: every date-$t$ feasible payoff arises by stopping now or by deferring (\texttt{DMC.Paper.stopping\_max\_le}). That dichotomy \emph{is} the formal content of Assumption~1, so what remains assumed is now the paper's own sentence rather than the algebraic conclusion. \texttt{DMC.SingleShot.stopping} still postulates the whole equation.
  \par\smallskip\emph{What would discharge it.} Assumption~1 lives \emph{below} this abstraction boundary (it constrains the game primitives, while $\mathcal R_t^{\Gamma}$ and $\mathcal V_t^{\Gamma}$ enter as unrelated parameters), so exhaustiveness cannot be derived here without relocating the assumption. Discharging it means modelling the allocation/termination structure of the game --- including the private-history argument the paper defers to its appendix. The second equality (Snell envelope over stopping times) additionally needs optional stopping, which the paper cites to Peskir--Shiryaev.
\end{itemize}
\begin{itemize}
